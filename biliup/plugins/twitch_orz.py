...
class TwitchOrz(DownloadBase, BatchCheck):
    ...
    async def acheck_stream(self, is_check=False):
        ...
        if not user:
            logger.warning(f"{TwitchOrz.__name__}: {self.url}: 获取错误", exc_info=True)
            return False
        ...
        if self.downloader == 'ffmpeg':
            ...
            # twitch_proxy_playlist = config.getConfig("twitch_proxy_playlist")
            # twitch_proxy_playlist_str = f" --twitch-proxy-playlist https://lb-eu.cdn-perfprod.com " if twitch_proxy_playlist else ''
            url = str(self.url).replace('.orz.tv', '.tv')
            # cmd = f"streamlink --plugin-dir streamlink_plugins --twitch-proxy-playlist https://lb-eu.cdn-perfprod.com {url}"
    ...
    async def abatch_check(check_urls: List[str]) -> AsyncGenerator[str, None]:
        ...
        for index, data in enumerate(gql):
            ...
            if not user:
                logger.warning(f"{TwitchOrz.__name__}: {check_urls[index]}: 获取错误")
                continue
            elif not user['stream'] or user['stream']['type'] != 'live':
                continue
