...
class TwitchM3U8Parser(M3U8Parser[TwitchM3U8, TwitchHLSSegment, HLSPlaylist]):
    ...
    def parse_tag_ext_x_twitch_prefetch(self, value):
        ...
        duration = last.duration if last.prefetch else sum(segment.duration for segment in segments) / float(
            len(segments))
            ...
            schema=validate.Schema(
                validate.any(
                    ...
                    validate.all(
                        ...
                        validate.get("data"),
                        validate.transform(
                            lambda data: ("token", *data) if data is not None else ("token", None, None)),
                    ),
                ),
            ),
        )
